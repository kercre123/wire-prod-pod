...
func IntentPass(req interface{}, intentThing string, speechText string, intentParams map[string]string, isParam bool, justThisBotNum int) (interface{}, error) {
	...
	if !isIntentGraph {
		...
		logger.Logger("Bot " + strconv.Itoa(justThisBotNum) + " Intent Sent: " + intentThing)
		if isParam {
			logger.Logger("Bot "+strconv.Itoa(justThisBotNum)+" Parameters Sent:", intentParams)
		} else {
			logger.Logger("No Parameters Sent")
		}
		return r, nil
	} else {
		...
		for _, c := range customIntentJSON {
			for _, v := range c.Utterances {
				if strings.Contains(voiceText, strings.ToLower(strings.TrimSpace(v))) {
					logger.Logger("Custom Intent Matched: " + c.Name + " - " + c.Description + " - " + c.Intent)
					var intentParams map[string]string
					var isParam bool = false
					if c.Params.ParamValue != "" {
						logger.Logger("Custom Intent Parameter: " + c.Params.ParamName + " - " + c.Params.ParamValue)
					...
					if len(args) == 0 {
						logger.Logger("Executing: " + c.Exec)
						customIntentExec = exec.Command(c.Exec)
					} else {
						logger.Logger("Executing: " + c.Exec + " " + strings.Join(args, " "))
					...
					logger.Logger("Custom Intent Exec Output: " + strings.TrimSpace(string(customOut)))
