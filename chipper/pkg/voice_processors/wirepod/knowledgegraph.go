...
func InitHoundify() {
	...
	if os.Getenv("HOUNDIFY_CLIENT_KEY") == "" {
		logger.Logger("Houndify Client Key not provided.")
		houndEnable = false
	}
	if houndEnable {
		...
		logger.Logger("Houndify for knowledge graph initialized!")
	}
}
...
func (s *Server) ProcessKnowledgeGraph(req *vtt.KnowledgeGraphRequest) (*vtt.KnowledgeGraphResponse, error) {
	...
	logger.Logger("(KG) Bot " + strconv.Itoa(justThisBotNum) + " request served.")
	if err := req.Stream.Send(&kg); err != nil {
		return nil, err
	}
	return &vtt.KnowledgeGraphResponse{
