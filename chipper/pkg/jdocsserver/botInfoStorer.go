...
func WriteToIni(botName string) {
	...
	fullPath, _ := os.Getwd()
	fullPath = strings.TrimSuffix(fullPath, "/wire-prod-pod/chipper") + "/.anki_vector/"
	if err != nil {
	...
	for _, robot := range robotSDKInfo.Robots {
		...
		if !matched {
			...
			logger.Logger("Getting session cert from Anki server")
	...
	logger.Logger("JSON to ini done")
...
func StoreBotInfo(ctx context.Context, thing string) {
	...
	if appendNew {
		logger.Logger("Adding " + botEsn + " to bot info store")
		robotSDKInfo.Robots = append(robotSDKInfo.Robots, struct {
			Esn       string `json:"esn"`
			IPAddress string `json:"ip_address"`
			GUID      string `json:"guid"`
