...
func (s *JdocServer) WriteDoc(ctx context.Context, req *jdocspb.WriteDocReq) (*jdocspb.WriteDocResp, error) {
	logger.Logger("Jdocs: Incoming WriteDoc request, Item to write: " + req.DocName + ", Robot ID: " + req.Thing)
...
func (s *JdocServer) ReadDocs(ctx context.Context, req *jdocspb.ReadDocsReq) (*jdocspb.ReadDocsResp, error) {
	...
	logger.Logger("Jdocs: Incoming ReadDocs request, Robot ID: " + req.Thing + ", Item(s) to return: ")
	logger.Logger(req.Items)
	...
	if strings.Contains(req.Items[0].DocName, "vic.AppToken") {
		if _, err := os.Stat(JdocsPath + strings.TrimSpace(req.Thing) + "-vic.AppTokens.json"); err != nil {
			logger.Logger("App tokens jdoc not found for this bot, trying bots in TokenHashStore")
			matched := false
			for num, pair := range tokenserver.TokenHashStore {
				if strings.EqualFold(pair[0], ipAddr) {
					...
					if err != nil {
						logger.Logger("Error writing token hash to vic.AppTokens.json")
						logger.Logger(err)
					}
					err = tokenserver.SetBotGUID(esn, pair[1], pair[2])
					if err != nil {
						logger.Logger("Error writing token hash to " + tokenserver.BotInfoFile)
						logger.Logger(err)
					}
					logger.Logger("ReadJdocs: bot " + esn + " matched with IP " + ipAddr + " in token store")
					matched = true
			...
			for num, pair := range tokenserver.SessionWriteStoreNames {
				if ipAddr == strings.Split(pair[0], ":")[0] {
					sessionMatched = true
					fullPath, _ := os.Getwd()
					fullPath = strings.TrimSuffix(fullPath, "/wire-prod-pod/chipper") + "/.anki_vector/" + pair[1] + "-" + esn + ".cert"
			...
			if !matched {
				if !isAlreadyKnown {
					logger.Logger("Bot was not known to wire-pod, creating token and hash (in ReadDocs)")
				...
				logger.Logger("Bot not found in any store, providing global GUID")
				return &jdocspb.ReadDocsResp{
					Items: []*jdocspb.ReadDocsResp_Item{
						{
							Status: jdocspb.ReadDocsResp_CHANGED,
	...
	for _, item := range req.Items {
		...
		if err != nil {
			...
			logger.Logger("Deprecated jdoc format found, converting to proper jdoc")
