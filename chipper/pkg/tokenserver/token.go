...
func SetBotGUID(esn string, guid string, guidHash string) error {
	...
	for num, robot := range robotInfo.Robots {
		if strings.EqualFold(esn, robot.Esn) {
			...
			logger.Logger(robot.Esn + " sucessfully activated with wire-pod")
			matched = true
			break
		}
	}
...
func RemoveFromSecondStore(s [][4]string, index int) {
	logger.Logger("Removing " + s[index][0] + " from temporary token-hash store")
...
func CreateJWT(ctx context.Context, skipGuid bool) *tokenpb.TokenBundle {
	...
	logger.Logger("Current time: " + currentTime)
	logger.Logger("Token expires: " + expiresAt)
	...
	if err == nil {
		logger.Logger("Found ESN for target " + ipAddr + ": " + esn)
		requestorId = "vic:" + esn
		if !skipGuid {
		...
		logger.Logger("ESN not found in store, this bot is new")
		if !skipGuid {
			logger.Logger("Adding " + ipAddr + " to TokenHashStore")
			guid, tokenHash, _ := CreateTokenAndHashedToken()
	...
	if secondary {
		...
		logger.Logger("Secondary client: " + secondaryGUID)
	}
